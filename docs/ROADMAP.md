...
|---|---|---|
| D-1 | ~~**Build the reviewer queue, or adopt ROOST Coop.**~~ **Answered 2026-09-08: build, and it is built.** Coop 1.0 ships queues, policy-bound actions, an audit log and CyberTipline submission, and none of that is the part that was hard. What Guardian's console does that a generic queue does not is refuse: no T3 without two people, no proposal without an excerpt rendered to that reviewer, no report until somebody designates which account it is about, and every string through the accusation guard. Those are the product. Coop stays a candidate for the submission transport in phase 3 if the ESP client turns out to be more work than it looks. |
| D-2 | ~~**Scope to the stranger and financial pattern, or build the known-contact path.**~~ **Answered 2026-09-08: scope it, explicitly.** The known-contact population is large and on the carried-out-threat number worse, and not one decisive signal fires on it: no new account, no catfish profile, no migration ask, often no age gap. Covering it needs a different signal set and labelled data nobody has. It is now written into DESIGN.md 12 and the model card's out-of-scope list as a decision rather than an open question, with the sentence that has to travel beside every recall number: that recall is over the stranger and financial pattern, not over sextortion. |
