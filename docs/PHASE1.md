...
- ~~**Redis stream retention.**~~ Closed as ROADMAP S-4. The sweep has a `streams` step that runs `XTRIM MINID` at the same 24 hour cutoff the T0 text clear uses. Exact, not `MINID ~`: the tilde stops at a node boundary, which is right for a size cap and wrong for a deletion deadline. The dead-letter keys carry no text and keep their count cap.
